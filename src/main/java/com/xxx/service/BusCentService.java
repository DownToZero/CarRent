...
import com.xxx.mapper.BusRentMapper;
...
@Service
public class BusCentService extends BaseService<BusRent,Integer> {
    ...
    public Map<String,Object> findall(BusRentQuery busRent){
        ...
        //调用方法查询所有的信息
        List<BusRent> slist = busRentMapper.selectParams(busRent);
        //开始分页
    ...
    public void save(BusRent busRent, Integer time, HttpServletRequest req){
        //1.参数校验
        checkParams(busRent.getRentid(),busRent.getIdentity(),busRent.getCarnumber());
        // 2.设置相关参数默认值
        busRent.setRentflag(0);//0-未分配，1-已分配

        busRent.setRentid(makeDh());
        ...
        System.out.println(time+"归还时间");
        busRent.setReturndate(addAndSubtractDaysByGetTime(new Date(),time));

//      System.out.println(busRentMapper.selectRentprice(busRent.getCarnumber())+"这是出租价格");
        busRent.setPrice(busRentMapper.selectRentprice(busRent.getCarnumber()));

        busRentMapper.updateBuscarIsrenting(busRent.getCarnumber());
        //3.执行添加 判断结果
        AssertUtil.isTrue(busRentMapper.insertSelective(busRent)<1,"添加失败了");
    }
    ...
    private void checkParams(String rentid, String identity, String carnumber) {
        ...
        List<Map<String, Object>> list = busCustomerMapper.findAllidentity();
        boolean flag=true;
    ...
    public void removeIds(String [] rentids){
        ...
        AssertUtil.isTrue(busRentMapper.deleteBatch(rentids)<1,"批量删除失败了");
    }
    ...
    public BusRent selectByPrimaryKey(String string){
        return busRentMapper.selectByPrimaryKey(string);
    }
